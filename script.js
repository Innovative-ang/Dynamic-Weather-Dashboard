// CONFIG
const API_KEY = '3a7671a258df13f0ad1344a1ca866c8a'; 
let units = 'metric'; // 'metric' | 'imperial'

// Cache exact location to keep conditions stable across unit toggles
let lastCoords = null;   // { lat, lon }
let lastCityName = null; // display name only

// Timers for live clocks
let __istTimer = null;
...
// WEATHER BY CITY (fallback when Enter without selecting suggestion)
...
function renderCurrent(data) {
  ...
  const tz = data.timezone || 19800; // fallback IST offset
  
  const cityLocalSec = currentUTC + tz;
  const localHour = new Date(cityLocalSec * 1000).getUTCHours();

  console.log('City:', data.name, 'Timezone:', tz, 'LocalHour:', localHour);
  console.log("Browser time:", new Date().toLocaleString());


  // Set theme based on hour
  if (localHour >= 6 && localHour < 19) {
    // Day theme
    document.body.style.backgroundImage = "url('background.jpg')";
    document.body.style.backgroundSize = "cover";
    document.body.style.backgroundPosition = "center";
    document.getElementById("night-indicator").textContent = ""; // Hide night text
    document.getElementById("moon-svg").style.display = "none";
    document.querySelector(".stars").style.display = "none";
    document.getElementById("wIconImg").style.display = "block";
    document.getElementById("wIconImg").src = data.weather?.[0] ? `https://openweathermap.org/img/wn/${data.weather[0].icon}@2x.png` : "";
    document.getElementById("wIconImg").alt = data.weather?.[0]?.description || "";
  } else {
    // Night theme
    document.body.style.backgroundImage = "url('nighttheme.jpg')";
    document.body.style.backgroundSize = "cover";
    document.body.style.backgroundPosition = "center";
    document.getElementById("night-indicator").textContent = "Night";
    document.getElementById("moon-svg").style.display = "block";
    document.querySelector(".stars").style.display = "block";
    document.getElementById("wIconImg").style.display = "none";
  }
...
function updateNightIndicator() {
  const hour = new Date().getHours();
  const nightDiv = document.getElementById('night-indicator');
  if (!nightDiv) return;
  if (hour >= 19 || hour < 6) {
    nightDiv.textContent = "Night";
  } else {
    nightDiv.textContent = "";
  }
}
